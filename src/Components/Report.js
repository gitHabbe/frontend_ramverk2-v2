...
class Report extends Component {
    ...
    async componentDidMount() {
        try {
            const kmomData = await axios.get(`${process.env.REACT_APP_API_URL}/reports/${this.state.kmom}`);
    ...
    async componentDidUpdate(prevProps) {
        ...
        if (prevNum !== curNum) {
            try {
                const kmomData = await axios.get(`${process.env.REACT_APP_API_URL}/reports/${curNum}`);
                this.setState({ kmomData });
            } catch (error) {
                console.log('TCL: Report -> }catch -> error', error);
                // Ugly slution but it works for now
